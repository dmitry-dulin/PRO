...
define('dataManager', ['jquery', 'knockout', 'queryApi', 'sessionApi', 'entityApi', 'QueryResults', 'arrayLogo'], function ($, ko, queryApi, sessionApi, entityApi, QueryResults, arrayLogo) {
    ...
    var self = this;var dataParser = new DataParser();self.getProMatches = function (entityType, entityId, callback) {
      var attributes = { matchType: '4575b5ba-82c5-4fc2-bacf-75bac90a50e7', awayTeam: '126b8dac-217f-4619-b9e0-efa91e9c9b47', homeTeam: 'e2e56898-14e9-4ddc-bdf9-860cd070491a', venue: '2a3b89c7-8b06-4290-bd4b-1031d6b8e8c7', refereePosition: 'c72177a1-9bcf-47d9-98d4-b7363aa610cc', refereeAccept: 'a95f5777-ea33-47e5-b546-a066eacb93a5' };console.log(sessionApi.gettemplatesessionsdata);entityApi.getgroup('11d4309d-cc44-493e-8a42-221b225bd1e8').then(function (group) {
        sessionApi.gettemplatesessionsdata('05dca773-4b20-43f0-8cb8-fa9d6ff83c63', { dateRangeType: "Offset", dateOffset: 60 }, true).then(function (data) {
          if (callback && typeof callback === "function") callback(dataParser.matchParse(data, attributes, group));
        }, function (error) {
  ...
  function DataParser() {
    ...
    self.getQueryParameters = function (entityType, entityId, query, fromDate, toDate) {
      ...
      var dateOffset = utils.getDateOffset(fromDate, toDate);var dateRange = { dateRangeType: 3, dateOffset: dateOffset, date: utils.displayString(toDate, "dash") };return $.extend({ entityType: entityType, entityId: entityId, queryName: query }, dateRange);
    };
    self.matchParse = function (data, attributes, allReferee) {
      var result = [];data.forEach(function (item, index, arr) {
        ...
        });result.push(new Match(item['sessionDetails']['id'], utils.getValueByIndex(item['data'], attributes['matchType']), item['sessionDetails']['start'], utils.getValueByIndex(item['data'], attributes['venue']), utils.getValueByIndex(item['data'], attributes['homeTeam']), utils.getValueByIndex(item['data'], attributes['awayTeam']), refereeList));
      });
      return utils.setTeamLogo(result);
    };
    self.parseQuery = function (queryParameters, attributes, flags) {
      var xhr = queryApi.getquery(queryParameters);var promise = xhr.then(function (resultData) {
        var results = QueryResults.create(resultData);var data = [];data = new Default().get(results, attributes);data = utils.setTeamLogo(data);return data;
      }).fail(function (error) {
        console.error("Request Failed: " + JSON.stringify(error));return [];
    ...
    var Baseline = function Baseline() {
      ...
      this.get = function (results, attributes) {
        var columns = results._data.columns;$.each(attributes, function (i, attr) {
          if (attr.index) {} else if (attr.baseline_attribute_id) {
            attr.index = utils.getAttributeIdIndexesLatestDate(columns, attr.baseline_attribute_id, attr.aggregation_type)[0];
          } else {
            attr.index = utils.getHeaderIndexes(columns, attr.name)[0];
          }
        });var rows = results._data.rows;var dataArray = [];$.each(rows, function (i, row) {
          var dataRow = new Object();dataRow.aggregation = 'Baseline';$.each(attributes, function (j, attr) {
            dataRow[attr.attribute] = getCell(row, attr);
          });dataArray.push(dataRow);
        });return dataArray;
      };
  ...
  function Utils() {
    ...
    return { getFirstYearDay: function getFirstYearDay() {
      ...
      }, setTeamLogo: function setTeamLogo(dataArray) {
  ...
  function Match(id, matchType, date, vanue, homeTeam, awayTeam, gameData) {
    this.id = id, this.matchType = matchType, this.date = date, this.venue = vanue ? vanue : ' - ', this.homeTeam = homeTeam, this.homeTeamLogo = "", this.awayTeam = awayTeam, this.awayTeamLogo = "", this.isVisibleInfo = ko.observable(false), this.gameData = gameData;this.color = setColor(gameData);this.assigned = gameData.length == 4 ? true : false;
  };function Referee(id, refereePosition, accept, allReferee) {
    this.id = id, this.refereePosition = refereePosition, this.status = "Assigned", this.statusColor = "green", this.name = getRefereeName(id, allReferee), this.accept = accept;
  }
